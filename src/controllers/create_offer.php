...
if (include(BASE_DIR . "/src/controllers/_offer_form.php")) {
    foreach ($_FILES['images']['tmp_name'] as $index => $tmp_name) {
        if (!in_array(mime_content_type($tmp_name), ['image/jpeg', 'image/png', 'image/jpg'])) {
            throw new Exception("Impossible de créer cette proposition!", 500);
        }
    }
    // Validate images count
    if (0 == array_reduce($_FILES['images']['tmp_name'], function ($memo, $name) {
            return $memo + empty($name) ? 0 : 1;
        }, 0)) {
